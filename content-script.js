function cleanup() { const url = new URL(location.href); const params = new URLSearchParams(url.search.replace('?', ''));
...
function restoreHistory() {
    ...
    if (history || !donor) { return; }
    const linkText = donor.textContent || 'History';
    ['trending-guide-item', 'what_to_watch-guide-item'].some(id => insertMenuItemAfter(document.querySelector(`#${id}`), 'history-guide-item', linkText, 'https://www.youtube.com/feed/history', 'guide-history-icon'));
}

function restoreMyChannel() {
    const donor = document.querySelector('#creation-live-menu-item');
    if (document.querySelector('#my-channel-guide-item') || !donor) { return; }
    const url = donor.href.replace('studio.youtube.com', 'youtube.com').replace('/livestreaming', '');
    if (url.indexOf('create_channel') >= 0) { return; }
    const linkText = chrome.i18n.getMessage('my_channel_menu_title');
    ['what_to_watch-guide-item', 'trending-guide-item'].some(id => insertMenuItemAfter(document.querySelector(`#${id}`), 'my-channel-guide-item', linkText, url, 'guide-my-channel-icon'));
}
chrome.runtime.sendMessage({ type: 'getOptions' }, (options) => {
    function restoreShowMoreButton() {
        ...
        if (!actionPanel || showMore) { return; }
    ...
    document.documentElement.addEventListener('click', e => {
        ...
        if (method === currentPageFixMethod) { return; }
        const spLinkSelector = isOldInterface ? 'spf-link' : 'yt-simple-endpoint';
        while (node && node.parentElement) { node.classList.remove(spLinkSelector);
            node = node.parentElement; }
    }, true);
    const cleanupAndRestore = () => { cleanup();
